	...
	function getWeather(cityName){
		...
		$.getJSON("http://api.openweathermap.org/data/2.5/forecast?q="+cityName+"&appid=5c818fece92856e67c360f488be116cc&lang=kr&units=metric",function(forecast){	
			...
			for(var i=0; i<arr.length; i++){
			 	...
			 	var text = "<div>";
			 	text += ("<span id='weatherInfo_"+i+"' class='weather_info' draggable='true' ondragstart='weather_drag("+i+")' value='"+str+"'>");
			 	text += (arr[i].date+" 날씨</span></div>");
...
//=============================================이벤트=============================================
	...
	//도시정보 셀렉트창 띄우기
	title_CityInfo.on("click", function () {
		wrapper_CityInfo_List.toggle(500);
	});

	//계산기 띄우기
	title_Calculator.on("click", function () {
		calculator.toggle(500);
	});

	//환율계산기 띄우기
	title_ExchangeRate.on("click", function () {
		wrapper_ExchangeRate.toggle(500);
	});


	//명소 리스트 띄우기
	title_hotPlace_List.on("click", function () {
		wrapper_hotPlace_List.toggle(500);
	});

	//맛집 리스트 띄우기
	title_restaurant_List.on("click", function () {
		wrapper_restaurant_List.toggle(500);
	});

	//쇼핑 리스트 띄우기
	title_shopping_List.on("click", function () {
		wrapper_shopping_List.toggle(500);
	});

	//날씨 리스트 띄우기
	title_weather_List.on("click", function () {
		wrapper_weather_List.toggle(500);
	});
	...
	//선택 도시의 좌표 얻어오기
	function getGeocode(geocoder, cityName) {

		geocoder.geocode({ 'address': cityName }, function (result, status) {
			console.log(result);
			console.log(status);
